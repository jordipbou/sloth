...
void _start_quotation(X* x) {
	...
	compile(x, find_word(x, "(QUOTATION)"));
...
void _constant(X* x) { 
	...
	tlen = cfetch(x, pop(x));
	header(x, tok, tlen);
	literal(x, v); compile(x, find_word(x, "EXIT"));
}
