...
typedef struct O { C t; C l; union { C c; void* p; } v; } O;
...
C S_dump_S(B* s, X* x) {
	C i = 0, t, n = 0;
	while (i < x->sp) { s += t = sprintf(s, "%ld ", x->s[i++].v.c); n += t; }
	return n;
}
...
#define TS(x) ((x->s[x->sp - 1]).v.c)
#define NS(x) ((x->s[x->sp - 2]).v.c)
#define NNS(x) ((x->s[x->sp - 3]).v.c)

void S_lit(X* x, C v) { x->s[x->sp++].v.c = v; x->s[x->sp].t = INT; }
...
C S_drop(X* x) { return x->s[--x->sp].v.c; }
B* S_dropB(X* x) { return x->s[--x->sp].v.p; }
